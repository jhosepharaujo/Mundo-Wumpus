...
public class Mapa {
	...
	Mapa(int dimensao){
		...
		this.y_player = 0;
		this.generateMapa();
		this.generatePositionPlayer();
		this.generatePositionHole();
	}
	...
	public void generatePositionPlayer()
	{
		mapa[this.getX_player()][this.getY_player()] = MapaObjetos.JOGADOR.getValor();
	}
	
	//======== GERAR POSICAO DOS BURACOS ========
	public void generatePositionHole()
	{
		/*
		 * gerar buraco random
		 * não gerar na posicao inicial do player
		 * não gerar no mesmo local
		 * */
		
		 
		int quantidade_buracos = this.getDimensao() - 1;
		for(int i=0;i<quantidade_buracos;i++) {
			if((randon_linha == this.getX_player()) &&(randon_coluna == this.getY_player()))
			{
				this.generatePositionHole();
			}
			mapa[randon_linha][randon_coluna] = MapaObjetos.BURACO.getValor();	
		}
	}
	
	private int getRandomCoordenadaValue()
	{
		int quantidade_buracos = this.getDimensao() - 1;
		Random random = new Random();
		int coordenada = random.nextInt(quantidade_buracos);
		return coordenada;
	}
